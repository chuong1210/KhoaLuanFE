...
import type { Voucher, VoucherFormData, VoucherUsageDetail } from "@/types/voucher";
...
export const voucherService = {
  getVouchers: async (): Promise<Voucher[]> => {
    const response = await axiosInstance.get(`${VOUCHER_API}?sort_by=created_at`);
  ...
  createVoucher: async (data: VoucherFormData): Promise<void> => {
    ...
    const response = await axiosInstance.post(VOUCHER_API, payload);
  ...
  updateVoucher: async (id: string, data: Partial<VoucherFormData>): Promise<void> => {
    ...
    const response = await axiosInstance.put(`${VOUCHER_API}/${id}`, payload);
    if (response.data.status !== "success") {
