...
export default function EditShopPage() {
  ...
  useEffect(() => {
    if (shop) {
      ...
      // Set preview ảnh cũ
      if (shop.shopLogo) {
        setLogoPreview(shop.shopLogo.startsWith("http") ? shop.shopLogo : `${MEDIA_BASE_URL}/${shop.shopLogo}`);
      }
      if (shop.shopBanner) {
        setBannerPreview(shop.shopBanner.startsWith("http") ? shop.shopBanner : `${MEDIA_BASE_URL}/${shop.shopBanner}`);
      }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error.response?.data?.errors) {
        ...
        toast.error("Dữ liệu không hợp lệ", { description: "Vui lòng kiểm tra các trường báo đỏ." });
      } else {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: "shopLogo" | "shopBanner") => {
    ...
    return (
      <div className="p-6 max-w-5xl mx-auto">
        <Alert variant="destructive">
          ...
          <AlertDescription>Không thể tải thông tin cửa hàng. Vui lòng thử lại sau.</AlertDescription>
        </Alert>
        <Button variant="outline" className="mt-4" onClick={() => router.push("/dashboard/shop")}>Quay lại</Button>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-xl border shadow-sm">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => router.back()} className="hover:bg-orange-50 text-orange-600">
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa cửa hàng</h1>
              <p className="text-sm text-gray-500">Cập nhật thông tin hiển thị và liên hệ</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-lg border">
              ...
              <Switch
                ...
                onCheckedChange={(c) => setFormData({ ...formData, shopStatus: c })}
                className="data-[state=checked]:bg-[#FF6A00]"
              />
              <span className={cn("text-sm font-medium", formData.shopStatus ? "text-green-600" : "text-gray-500")}>
              ...
              <CardContent className="pt-6 space-y-5">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={cn("h-11 focus:ring-[#FF6A00]", hasError("ShopName") && "border-red-500")}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={formData.shopDescription} onChange={handleTextChange}
                    rows={4}
            ...
            <Card className="shadow-sm border-gray-200 overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-5">
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="shopEmail">Email <span className="text-red-500">*</span></Label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
                      <Input
                        id="shopEmail" name="shopEmail" type="email"
                        value={formData.shopEmail} onChange={handleTextChange}
                        className={cn("pl-9 h-11 focus:ring-[#FF6A00]", hasError("ShopEmail") && "border-red-500")}
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        value={formData.shopPhone} onChange={handleTextChange}
                        className={cn("pl-9 h-11 focus:ring-[#FF6A00]", hasError("ShopPhone") && "border-red-500")}
                      />
                    </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="shopAddress">Địa chỉ <span className="text-red-500">*</span></Label>
                  <Textarea
                    id="shopAddress" name="shopAddress"
                    value={formData.shopAddress} onChange={handleTextChange}
                    rows={2}
                    className={cn("resize-none focus:ring-[#FF6A00]", hasError("ShopAddress") && "border-red-500")}
                  />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      id="shopPersonalIdentifyId" name="shopPersonalIdentifyId"
                      value={formData.shopPersonalIdentifyId} onChange={handleTextChange}
              ...
              <CardContent className="pt-6 flex flex-col items-center">
                <div className="relative group">
                  <div className={cn(
                    ...
                    hasError("ShopLogo") && "border-red-400 ring-2 ring-red-200"
                  )}>
                    {logoPreview ? (
                  ...
                  {/* Edit Overlay */}
                  <label htmlFor="shopLogo" className="absolute inset-0 rounded-full bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                    <Upload className="h-8 w-8 text-white" />
                    <Input id="shopLogo" type="file" accept="image/*" onChange={(e) => handleFileChange(e, "shopLogo")} className="hidden" />
                  </label>

                  {logoPreview && (
                    <button type="button" onClick={() => handleRemoveImage("shopLogo")} className="absolute top-0 right-0 bg-red-500 text-white p-1 rounded-full shadow-md hover:bg-red-600">
                      <X className="h-3 w-3" />
                    </button>
                  )}
                </div>
                <p className="text-xs text-gray-500 mt-3 text-center">
                  Kích thước: 400x400px<br />Dung lượng &lt; 5MB
                </p>
            ...
            <Card className="shadow-sm border-gray-200 overflow-hidden">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className={cn(
                  ...
                  <Input id="shopBanner" type="file" accept="image/*" onChange={(e) => handleFileChange(e, "shopBanner")} className="absolute inset-0 opacity-0 cursor-pointer" />

                  {bannerPreview && (
                    <button type="button" onClick={() => handleRemoveImage("shopBanner")} className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded shadow-md hover:bg-red-600 z-10">
                      <X className="h-3 w-3" />
                    </button>
                  )}
                </div>
                ...
                {bannerPreview && (
                  <div className="space-y-2 animate-in fade-in">
                    <Label htmlFor="bannerType" className="text-xs font-semibold text-gray-500 uppercase">Loại hiển thị</Label>
                    <Select value={formData.bannerType} onValueChange={(val) => setFormData({ ...formData, bannerType: val })}>
        ...
        <div className="flex items-center justify-end gap-4 pt-4 border-t">
          <Button variant="outline" onClick={() => router.back()} className="min-w-[100px]">
            Hủy bỏ
          </Button>
          <Button
            onClick={handleSubmit}
            disabled={updateMutation.isPending}
            className="min-w-[160px] text-white shadow-lg hover:shadow-xl transition-all"
